...
StorePromise.then((Store) => {
  ...
  async function searchForPodcasts(update, term, limit, loaded) {
    if (term === null || term === void 0 || loaded === true) {
      return;
    }
    ...
    if (responseJSON.results.length === 0) {
      ...
      const podcastFeedResultsPromises = responseJSON.results.map(async (searchResult) => {
        const podcast = {
  ...
  async function getResponseJSON(term, limit) {
