...
StorePromise.then((Store) => {
  customElement("pc-main-menu", ({constructing, update}) => {
    ...
    const desktopAndMenuClosed = state.showMainMenu && state.screenType === "DESKTOP";
    return html`
            <style>
  ...
  function menuItemClick(Store2, path) {
    navigate(Store2, path);
    if (Store2.getState().screenType === "MOBILE") {
      Store2.dispatch({
        type: "TOGGLE_SHOW_MAIN_MENU"
      });
    }
