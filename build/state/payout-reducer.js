...
export function PayoutReducer(state, action) {
  if (action.type === "SET_PODCAST_PAYMENTS_ENABLED") {
    const currentPodcast = state.podcasts[action.feedUrl];
    if (currentPodcast === null || currentPodcast === void 0) {
      return state;
    }
    const newPodcast = {
      ...currentPodcast,
  ...
  if (action.type === "SET_ETHEREUM_BALANCE_IN_WEI") {
    const newEthereumBalanceInWEI = action.ethereumBalanceInWEI;
    const payoutTargetInWEI = getPayoutTargetInWEI(state);
  ...
  if (action.type === "RESET_PODCAST_TIME_LISTENED_SINCE_PREVIOUS_PAYOUT") {
    const podcast = state.podcasts[action.feedUrl];
    const newPodcast = {
      ...podcast,
