export function DefaultReducer(state, action) {
  ...
  if (action.type === "DELETE_PODCAST") {
    const newCurrentEpisodeGuid = action.podcast.episodeGuids.includes(state.currentEpisodeGuid) ? "NOT_SET" : state.currentEpisodeGuid;
    const newPlaylist = state.playlist.filter((episodeGuid) => {
      return !action.podcast.episodeGuids.includes(episodeGuid);
    });
    const newCurrentPlaylistIndex = newCurrentEpisodeGuid === "NOT_SET" ? 0 : newPlaylist.indexOf(newCurrentEpisodeGuid);
    const newEpisodes = Object.entries(state.episodes).filter((entry) => {
    ...
    const newPodcasts = Object.entries(state.podcasts).filter((entry) => {
      return entry[1].feedUrl !== action.podcast.feedUrl;
    }).reduce((result, entry) => {
      return {
        ...result,
